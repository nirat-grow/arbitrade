...
async function scanNetwork(netName, rpcUrl, maxBlocks = 25, targetCount = 6) {
  ...
  for (let b = currentBlock; b > currentBlock - maxBlocks && results.length < targetCount; b--) {
    ...
    for (const tx of block.transactions) {
      ...
      // We want MULTICALL with 4 to 20 transfers!
...
async function main() {
  const targetNet = process.argv[2];
  const rpcs = {
  ...
  if (targetNet && rpcs[targetNet]) {
    const res = await scanNetwork(targetNet, rpcs[targetNet]);
    console.log(JSON.stringify({ [targetNet]: res }, null, 2));
  } else {
    for (const [net, rpc] of Object.entries(rpcs)) {
      const res = await scanNetwork(net, rpc, 15, 5);
      console.log(`=== ${net} DONE (${res.length} txs) ===`);
    }
  }
}
